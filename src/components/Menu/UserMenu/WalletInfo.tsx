...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: cakeBalance, fetchStatus: cakeFetchStatus } = useTokenBalance(tokens.mgc.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        {busdFetchStatus !== FetchStatus.Fetched ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(busdBalance, 18, 1)}</Text>
        )}
      </Flex>
      <Flex alignItems="center" justifyContent="space-between">
        ...
        {usdtFetchStatus !== FetchStatus.Fetched ? (
          ...
          <Text>{getFullDisplayBalance(usdtBalance, 18, 1)}</Text>
        )}
      </Flex>
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('MGC Balance')}</Text>
        {cakeFetchStatus !== FetchStatus.Fetched ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(cakeBalance, 9, 2)}</Text>
        )}
      </Flex>
